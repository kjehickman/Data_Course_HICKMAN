...
plot + transition_time(year) +
  labs(title = "Year: {frame_time}: After many attempts I finally found a way to include an angry macaroni penguin who is featured on every facet. 
       \nThis is to ensure that we understand how angry penguins can be about being small. 
       \nIf you pretend the flying feet-faces are insects, it makes the plots a little more interesting :D")

# this is an attempt to see if I can save this animation as a png
ggsave("./HICKMAN_Ugly_Plot.png", width = 8, height = 8)

colors()
 
# this is the plot that I can export as a jpg
plot2 <- ggplot(df, aes(x=bill_depth_mm,y=flipper_length_mm)) +
  annotation_custom(rasterGrob(image = jpeg, 
                               width = unit(1,"npc"), 
                               height = unit(1,"npc")),  
                    -Inf, Inf, -Inf, Inf) +

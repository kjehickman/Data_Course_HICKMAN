...
# ss is the name of the subset; iris denotes the df; seq() gives a selection for row specs; columns are blank
ss <- iris[(seq(2,150,2)), ]

# 2.  Create a new object called iris_chr which is a copy of iris, except where every column is a character class

# This function seems to default to the last line when printing... not sure if it's incorporating all of the lines.
...
chars2 <- c(iris$Sepal.Length, iris$Sepal.Width, iris$Petal.Length, iris$Petal.Width, iris$Species)
as.character(chars2)
class(iris$Sepal.Length)

iris$Sepal.Length[1]

# OR [preferred] this function seems to print all variables as character type but neither show in data
iris_chr <- as.character(iris)

for(l in iris) {as.character(iris[l, "Sepal.Length"])
print(l)
  }
class(iris$Sepal.Length)
...
big_area_iris <- data.frame(iris1[Sepal.Area > 20, ])
dim(big_area_iris)
